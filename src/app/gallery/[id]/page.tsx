...
export default async function GalleryPage({ params }: PageProps){
  ...
  const g = await prisma.gallery.findUnique({ where: { id }, include: { items: { include: { file: true } } } });
  if(!g) notFound();
  if(!canViewGallery(g, session)) notFound();

  type GalleryItemRecord = (typeof g.items)[number];
  type FileRecord = GalleryItemRecord["file"];
  type FileWithUrl = FileRecord & { _url: string };

  const filesWithUrl: FileWithUrl[] = await Promise.all(
    g.items.map(async (it: GalleryItemRecord) => {
      const f = it.file;
